...
class Discussion {
    ...
    public int getImportance(){
        return this.popularity * this.users;
    }
...
public class Forum {
    ...
    private static void forum(SLL<Discussion> health, SLL<Discussion> finance) {
        SLLNode<Discussion> first = finance.getFirst();
        SLLNode<Discussion> minImportance = finance.getFirst();

        while(first != null){
            if(first.element.getImportance() < minImportance.element.getImportance()){
                minImportance = first;
            }
            first = first.succ;
        }
        finance.delete(minImportance);

        SLLNode<Discussion> firstHealth = health.getFirst();
        SLLNode<Discussion> maxImportnace = health.getFirst();

        while (firstHealth != null){
            if(firstHealth.element.getImportance() > maxImportnace.element.getImportance()){
                maxImportnace = firstHealth;
            }
            firstHealth = firstHealth.succ;
        }

        finance.insertLast(maxImportnace.element);
    }
